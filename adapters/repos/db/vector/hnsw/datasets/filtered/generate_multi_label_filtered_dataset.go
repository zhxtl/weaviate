...
func main() {
	// CLI flags
	numVectors := flag.Int("numVectors", 1_000_000, "Number of vectors to process")
	...
	numLabels := flag.Int("numLabels", 5, "Number of labels to create")
	alpha := flag.Float64("alpha", 2.0, "Controls the power law distribution of label frequency.")

	flag.Parse()

	labelProportions := CalculatePowerLawDistributions(*numLabels, *alpha)
	...
	alpha_str := strconv.FormatFloat(*alpha, 'f', 1, 64)
	...
	for i, proportion := range labelProportions {
		...
		labelCounts[i] = count
		if i == 0 {
			labelCounts[i] += 5 //temporary fix
		}
	}
	fmt.Println("Label count:", labelCounts)
	...
	index_with_filters_save_path := "indexFilters-" + saveNumVectors[*numVectors] + "-" + strconv.FormatInt(int64(*numLabels), 10) + "-" + alpha_str + ".json"
	...
	queryLabelCounts := make([]int, *numLabels)
	for i, proportion := range labelProportions {
		count := int(proportion / 100 * float64(len(queryVectors)))
		queryLabelCounts[i] = count
		if i == 0 {
			queryLabelCounts[i] += 5 //temporary fix
		}
	}
	fmt.Println(queryLabelCounts)

	currentQueryLabel := 0
	for i, queryVector := range queryVectors {
		workerID := i % workerCount
		queryLabelCounts[currentQueryLabel]--
		if queryLabelCounts[currentQueryLabel] == 0 {
			currentQueryLabel++
		}
		queryFilters := make(map[int]int)
		queryFilters[0] = currentQueryLabel
	...
	query_vectors_with_filters_save_path := "queryFilters-" + saveNumVectors[*numVectors] + "-" + strconv.FormatInt(int64(*numLabels), 10) + "-" + alpha_str + ".json"
	...
	ground_truth_save_path := "filtered-recall-truths-" + saveNumVectors[*numVectors] + "-" + strconv.FormatInt(int64(*numLabels), 10) + "-" + alpha_str + ".json"
...
// CalculatePowerLawDistributions calculates and returns the power law distributions
// for a given number of labels and alpha value.
func CalculatePowerLawDistributions(numLabels int, alpha float64) []float64 {
	// Slice to hold the unnormalized weights
	...
	// Slice to hold the normalized proportions
	normalizedProportions := make([]float64, numLabels)

	// Normalize the weights to get the probability densities

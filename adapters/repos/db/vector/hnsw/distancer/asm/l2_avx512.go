...
//go:generate go run l2_avx512.go -out l2_avx512_amd64.s -stubs l2_avx512_stub_amd64.go
...
func main() {
	TEXT("L2AVX512", NOSPLIT, "func(x, y []float32) float32")
	...
	for i := 0; i < unroll; i++ {
		VMOVUPS(x.Offset(64*i), xs[i])
	}

	for i := 0; i < unroll; i++ {
		VSUBPS(y.Offset(64*i), xs[i], diff[i])
	}

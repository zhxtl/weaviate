...
func TestClient(t *testing.T) {
	...
	t.Run("when the server returns an error", func(t *testing.T) {
		...
		require.NotNil(t, err)
		assert.EqualError(t, err, "connection to  Gradient failed with status: 500 error: nope, not gonna happen")
	})

	t.Run("when Gradient key is passed using X-Gradient-Api-Key header", func(t *testing.T) {
		server := httptest.NewServer(&fakeHandler{t: t})
		defer server.Close()
		c := &gradient{
			apiKey:     "",
		...
		ctxWithValue := context.WithValue(context.Background(),
			"X-Gradient-Api-Key", []string{"some-key"})
	...
	t.Run("when Gradient key is empty", func(t *testing.T) {
		server := httptest.NewServer(&fakeHandler{t: t})
		defer server.Close()
		c := &gradient{
			apiKey:     "",
		...
		require.NotNil(t, err)
		assert.Equal(t, err.Error(), "Gradient API Key: no api key found "+
			"neither in request header: X-Gradient-Api-Key "+
			"nor in environment variable under GRADIENT_APIKEY")
	})

	t.Run("when X-Gradient-Api-Key header is passed but empty", func(t *testing.T) {
		server := httptest.NewServer(&fakeHandler{t: t})
		defer server.Close()
		c := &gradient{
			apiKey:       "",

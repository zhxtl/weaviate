...
const (
	workspaceIdProperty = "workspaceId"
	slugProperty        = "slug"
)
...
const (
	...
	DefaultApiEndpoint           = "api.gradient.ai"
	DefaultSlug                  = "bge-large"
)

var availableGradientModels = []string{
	DefaultSlug,
}
...
func (ic *classSettings) Validate(class *models.Class) error {
	...
	if workspaceId == "" {
		errorMessages = append(errorMessages, fmt.Sprintf("%s cannot be empty", workspaceIdProperty))
	}
	slug := ic.Slug()
	if model != "" && !ic.validateGradientSetting(slug, availableGradientModels) {
		errorMessages = append(errorMessages, fmt.Sprintf("wrong %s available model names are: %v", slugProperty, availableGradientModels))
	}
...
func (ic *classSettings) validateGradientSetting(value string, availableValues []string) bool {
...
func (ic *classSettings) getStringProperty(name, defaultValue string) string {
	...
	value, ok := ic.cfg.ClassByModuleName("text2vec-gradient")[name]
	if ok {
...
func (ic *classSettings) WorkspaceId() string {
	return ic.getStringProperty(workspaceIdProperty, "")
}

func (ic *classSettings) Slug() string {
	return ic.getStringProperty(slugProperty, DefaultSlug)
}

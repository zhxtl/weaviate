...
// Note MosaicML sets up their API differently than OAI / Cohere
const (
	DefaultMosaicMLModel         = "instructor-large/v1/predict"
...
var (
	availableMosaicMLModels = []string{"instructor-large/v1/predict", "instructor-xl/v1/predict"}
...
func (cs *classSettings) Model() string {
	return cs.getProperty("model", DefaultMosaicMLModel)
}
...
func (cs *classSettings) Validate(class *models.Class) error {
	...
	model := cs.Model()
	if !cs.validateMosaicMLSetting(model, append(availableMosaicMLModels)) {
		return errors.Errorf("wrong MosaicML model name, available model names are: %v", availableMosaicMLModels)
	}
	truncate := cs.Truncate()
	if !cs.validateMosaicMLSetting(truncate, availableTruncates) {
...
func (cs *classSettings) validateMosaicMLSetting(value string, availableValues []string) bool {
